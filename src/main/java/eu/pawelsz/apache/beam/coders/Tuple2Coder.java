...
public class Tuple2Coder<T0, T1> extends StandardCoder<Tuple2<T0, T1>> {
    ...
    public static <T0, T1> List<Object> getInstanceComponents(Tuple2<T0, T1> exampleValue) {
        return Arrays.asList(exampleValue.f0, exampleValue.f1);
    }
...
//        LOG.info("Tuple2Coder("+t0Coder.getClass().getName()+", "+t1Coder.getClass().getName()+")");
    ...
    @Override
    public Tuple2<T0, T1> decode(InputStream inputStream, Context context) throws CoderException, IOException {
        Context nestedContext = context.nested();
